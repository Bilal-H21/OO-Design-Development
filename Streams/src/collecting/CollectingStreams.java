...
public class CollectingStreams {
	...
	public static void main(String[] args) {
		...
		//populate an unmodifiable list of orders for testing
		List<OrderLine> orders = List.of(new OrderLine("Milk", 99, 2), new OrderLine("Cheese", 350, 1), new OrderLine("Bread", 130, 3), new OrderLine("Milk", 99, 1));
		 ...
		 */
		double averageUnitPrice = orders.stream()
				.collect(averagingInt(OrderLine :: getUnitPrice));
		System.out.println("Average unit price of all orders is: " + averageUnitPrice 
				+ " pence.");
		...
		/* TASK 3. Ensure the Name class is within your project and then use List.of to create and populate a
		...
		Map<Integer, String> lengthNameMap = names.stream()
			    .collect(toMap(n -> n.getFirstName().length() + n.getFamilyName().length(), 
			                   n -> n.getFirstName() + " " + n.getFamilyName(),
			                   (a, b) -> a + "; " + b));

			System.out.println("Length to Full Name Map: " + lengthNameMap);
